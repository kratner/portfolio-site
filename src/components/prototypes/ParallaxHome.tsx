    ...
    const handleScroll = () => {
      if (parallaxContainerRef.current) {
        ...
        const elementTransforms = perspectives.map((perspective, index) => {
          const translateY =
            ((scrollTop - containerTop) / containerHeight) * perspective;
          return `translateY(${translateY}px)`;
        });

        if (
          parallaxContainerRef.current.children.length === perspectives.length
        ) {
          Array.from(parallaxContainerRef.current.children).forEach(
            (element, index) => {
              (element as HTMLElement).style.transform =
                elementTransforms[index];
            }
          );
        }
      }
